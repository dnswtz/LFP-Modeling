...
class MEA:
	...
	def create_mea_data(self, adj_A1 = var.adj_A1, adj_A2 = var.adj_A2, nodes=int(var.numNeurons)):
		...
		for neuron in adj_A1:
			...
			self.neural_posX = np.concatenate((self.neural_posX,[pos[0]]))
			self.neural_posY = np.concatenate((self.neural_posY,[pos[1]]))
	...
	def no_volume_conduction(self, sigma=var.sigma_cond, adj_A1=var.adj_A1, adj_A2=var.adj_A2):
		...
		for ecog_electrode in range(len(self.positions)):
			...
			x_reach = np.where((x-1.14289 < self.neural_posX)&(self.neural_posX <= x+1.14289))[0] #neuronID within x-1.14 and x+1.14
			y_reach = np.where((y-1.14289 < self.neural_posY)&(self.neural_posY <= y+1.14289))[0] #neuronID within y-1.14 and y+1.14

			print(x_reach)

			reach = np.intersect1d(x_reach,y_reach) # neuronID within reach of electrode (volume conduction neglected)
